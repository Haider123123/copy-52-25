...
export default function App() {
  ...
  const handleDeleteAppointment = (patientId: string, appId: string) => { if (patientId) { const patient = data.patients.find(p => p.id === patientId); if(!patient) return; updatePatient(patientId, { appointments: patient.appointments.filter(a => a.id !== appId) }); } else updateLocalData(prev => ({ ...prev, guestAppointments: prev.guestAppointments.filter(a => a.id !== appId) })); };
